...
#undef DefineException
	...
	struct Value {
		public:
			explicit Value(std::string key) : _key(std::move(key)) {}
	...
	struct FunctionValue : public Value {
		...
		public:
			FunctionValue(const std::string &key, const FValueFNEasy &fn)
					: Value(key), _value([fn](const StringProperties &, std::deque<std::string> val) { return fn(val); }) {}

			FunctionValue(const std::string &key, FValueFN fn) : Value(key), _value(std::move(fn)) {}
		...
		class VariableTransform {
			public:
				static ValType transform(const VariableType &) {
					throw exceptions::InvalidValueTypeException("Could not convert from " + std::string(typeid(VariableType).name()) + " to " + std::string(typeid(impl::ValType).name()));
				}
		};
